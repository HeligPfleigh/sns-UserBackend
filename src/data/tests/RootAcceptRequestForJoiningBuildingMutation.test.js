...
describe('RootAcceptRequestForJoiningBuildingMutation', () => {
  beforeEach(async () => {
    // setup db
  ...
  test('should get error if user is not admin', async () => {
    ...
    const query = `
      mutation M { 
        acceptRequestForJoiningBuilding(buildingId:"${buildingId}",userId:"${userIdB}") {
          _id
          username
        }
      }
  ...
  test('should get user data', async () => {
    ...
    const context = getContext({});
    const result = await graphql(schema, query, rootValue, context);
    expect(result.data.acceptRequestForJoiningBuilding).toEqual({
      _id: userIdB,
